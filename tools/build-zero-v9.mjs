...
function replaceLineFunction(name,fn){
  const code=typeof fn==='function'?fn.toString():fn,lines=src.split('\n');let n=0;
...
const ptextLine=src.split('\n').find(line=>line.startsWith('function ptext('));
if(!ptextLine)throw new Error('ptext not found');src=src.replace(ptextLine,ptextLine+'\n'+fitPtext.toString());
...
src=src.replace(/function card\(x,y,w,h,accent,title,sub,done=false\)\{.*?\}function factEntry/,card.toString()+'function factEntry');

function menuBackdrop(seed=0){clear();rect(0,0,canvas.width,canvas.height,'#151d31');for(let i=0;i<44;i++){const n=hash(`star:${seed}:${i}`),x=n%canvas.width,y=(n>>>9)%canvas.height,s=(n>>>18)%5===0?2:1;rect(x,y,s,s,(n>>>22)%3===0?P.sand:P.gray);}for(let y=54;y<canvas.height;y+=32)rect(0,y,canvas.width,1,'#ffffff08');}
function bandColor(w){return [P.sand,P.mint,P.ice,P.violet,P.rust,P.red][Math.max(0,(w||1)-1)]||P.sand;}
const clearNeedle="function clear(){hits=[];rect(0,0,canvas.width,canvas.height,P.deep);}";
if(!src.includes(clearNeedle))throw new Error('clear helper not found');src=src.replace(clearNeedle,clearNeedle+menuBackdrop.toString()+bandColor.toString());
...
function syncMusic(){if(music){musicV9.setScene(screen==='nil'?6:world);musicV9.start(screen==='nil'?6:world);}else musicV9.stop();}
replaceLineFunction('syncMusic','const musicV9=createMusicV9(audio);'+syncMusic.toString());
