...
- Never certify puzzle difficulty from hand-written metadata such as `reasoningSteps`, a difficulty label, or operand size. For generated constraint puzzles, enumerate the candidate solution space, require a unique intended solution, measure how many clues actually reduce that space, and gate higher bands on increasing search-space/constraint complexity. A test that merely asserts the authored difficulty number is not a difficulty test.
- High bands must retain multiple mathematical puzzle families (constraint systems, graphs/invariants, strategy games, equivalence/proof) rather than becoming a worksheet of missing-number equations.
